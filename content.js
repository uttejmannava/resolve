...
if (url.includes('/problemset/')) {
    chrome.storage.sync.get("hideDifficulty", (data) => {
        ...
        const difficulty_spans = Array.from(document.querySelectorAll("span"))
            .filter(span => ["Medium", "Easy", "Hard"].includes(span.textContent.trim()));
        
        difficulty_spans.forEach(span => {
            span.textContent = '🤷‍♂️';
        });

        const acceptance_spans = Array.from(document.querySelectorAll("span"))
            .filter(span => (/^\d+\.\d%$/.test(span.textContent.trim())));
        
        acceptance_spans.forEach(span => {
            span.textContent = '🤷‍♂️';
        });
